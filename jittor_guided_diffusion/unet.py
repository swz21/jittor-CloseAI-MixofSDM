...
class AttentionPool2d(nn.Module):
    ...
    def execute(self, x):
        ...
        x = jt.cat([x.mean(dim=-1, keepdim=True), x], dim=-1)  # NC(HW+1)
...
class SuperResModel(UNetModel):
    ...
    def __init__(self, image_width, image_height, in_channels, *args, **kwargs):
        super().__init__(image_width, image_height, in_channels * 2, *args, **kwargs)
    ...
    def __init__(
        ...
        if pool == "adaptive":
            ...
            self.out = nn.Sequential(
                ...
                SiLU(),
                AttentionPool2d(
                    ((image_width // ds), (image_height // ds)), ch, num_head_channels, out_channels
                ),
            )
        elif pool == "spatial":
            self.out = nn.Sequential(
    ...
    def execute(self, x, timesteps):
        ...
        for module in self.input_blocks:
            ...
            if self.pool.startswith("spatial"):
                results.append(h.to(x.dtype).mean(dim=(2, 3)))
        h = self.middle_block(h, emb)
        if self.pool.startswith("spatial"):
            results.append(h.to(x.dtype).mean(dim=(2, 3)))
            h = jt.cat(results, axis=-1)
            return self.out(h)
        else:
            h = h.astype(x.dtype)
            return self.out(h)

...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        if ddim_steps > 0:
            ...
            self.ddim_timesteps = make_ddim_timesteps(self.ddim_steps, self.num_timesteps)
            self.ddim_sigmas, self.ddim_alphas, self.ddim_alphas_prev = \
                make_ddim_sampling_parameters(alphacums=self.alphas_cumprod, ddim_timesteps=self.ddim_timesteps, eta=ddim_eta)
    ...
    def ddim_sample(
        ...
        b = x.shape[0]
        a_t = jt.full((b, 1, 1, 1), self.ddim_alphas[index])
        a_prev = jt.full((b, 1, 1, 1), self.ddim_alphas_prev[index])
        sigma_t = jt.full((b, 1, 1, 1), self.ddim_sigmas[index])
        sqrt_one_minus_at = jt.full((b, 1, 1, 1), self.ddim_sqrt_one_minus_alphas[index])

        # index = jt.array([index] * x.shape[0])
        # a_t = _extract_into_tensor(self.ddim_alphas, index, x.shape)
        # a_prev = _extract_into_tensor(self.ddim_alphas_prev, index, x.shape)
        # sigma_t = _extract_into_tensor(self.ddim_sigmas, index, x.shape)
        # sqrt_one_minus_at = _extract_into_tensor(self.ddim_sqrt_one_minus_alphas, x.shape)
        
        # eps = self._predict_eps_from_xstart(x, t, out["pred_xstart"])
        eps = (x - out["pred_xstart"] * a_t.sqrt()) / sqrt_one_minus_at
        # sigma = (
        #     eta
        ...
        noise = jt.randn_like(x)
        mean_pred = (
            out["pred_xstart"] * jt.sqrt(a_prev)
            + jt.sqrt(1 - a_prev - sigma_t ** 2) * eps
        )
        nonzero_mask = (
            (t != 0).float().view(-1, *([1] * (len(x.shape) - 1)))
        )  # no noise when t == 0
        sample = mean_pred + nonzero_mask * sigma_t * noise
        return {"sample": sample, "pred_xstart": out["pred_xstart"]}
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [
                ...
                frozen_out = jt.cat([model_output.detach(), model_var_values], dim=1)
